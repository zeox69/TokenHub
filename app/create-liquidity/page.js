  ...
  import { createCpmmPool } from "../services/raydium";
...
function copyText(text, label) {
    ...
    <div className="mt-5 space-y-4">
      <div className="rounded-2xl bg-black/30 p-4">
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          <button
            onClick={() => copyText(successData.txId, "tx")}
      ...
      <div className="rounded-2xl bg-black/30 p-4">
        <p className="text-sm text-gray-400">Pool ID</p>
        ...
        <div className="mt-2 flex items-center gap-3">
          ...
          <button
            onClick={() => copyText(successData.poolId, "pool")}
            className="rounded-xl bg-purple-600 px-4 py-2 font-bold"
          >
      ...
      <a
        href={`https://explorer.solana.com/tx/${successData.txId}?cluster=devnet`}
        target="_blank"
        rel="noopener noreferrer"
        className="mt-4 block w-full rounded-2xl bg-blue-600 py-4 text-center font-bold text-white transition hover:bg-blue-500"
      >
        View on Explorer
      </a>
    </div>
  ...
  Raydium liquidity creation currently works on Mainnet. Your website is using Devnet, so pool creation is temporarily unavailable until we switch networks.
</div>
                </div>
              </div>
            </div>
